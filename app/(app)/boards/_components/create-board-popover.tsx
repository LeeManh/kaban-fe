...
import { BoardBackgroundFlow } from "./board-background-flow";
import { COLOR_SWATCHES } from "./board-background-picker";

type CreateBoardView = "form" | "background";
...
export function CreateBoardPopover({ children }: { children: ReactElement }) {
  ...
  const [view, setView] = useState<CreateBoardView>("form");
  const createBoard = useCreateBoard();
  const { data: photos } = useUnsplashPhotos(open && view === "form");
  ...
  const backgroundValue = useWatch({ control: form.control, name: "background" });
  ...
  function handleBackgroundChange(value: string, photoDownloadLocation?: string) {
    setDownloadLocation(photoDownloadLocation);
    form.setValue("background", value, { shouldValidate: true, shouldDirty: true });
    setView("form");
  }
